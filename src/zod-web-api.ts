// Copied from https://raw.githubusercontent.com/ottersoft-x/zod-web-api/main/main.ts
// Only change is to getPaths to support `parent.index.child` instead of `parent[index].child`

import { type RefinementCtx, type ZodTypeAny, z } from "zod";
 ...
 * }
 */
export async function parseQueryAsync<
  TValue extends URLSearchParams | string | Request,
  TSchema extends ZodTypeAny,
>(value: TValue, schema: TSchema) {
...
export async function parseFormDataAsync<
  TValue extends FormData | Request,
  TSchema extends ZodTypeAny,
>(value: TValue, schema: TSchema) {
...
export async function safeParseFormDataAsync<
  TValue extends FormData | Request,
  TSchema extends ZodTypeAny,
>(value: TValue, schema: TSchema) {
  let formData: FormData;
 ...
 *
 */
export async function parseRequestAsync<TSchema extends ZodTypeAny>(
  request: Request,
  schema: TSchema,
) {
...
function getPaths(name: string): Array<string | number> {
  const path = name
    ? name.split(".").map((item) => {
        ...
        return Number.isInteger(parsed) ? parsed : item;
      })
    : [];
  return path;
}
...
export const zt = {
  ...
  json<TValue extends string | null | undefined, TSchema extends z.ZodTypeAny>(
    ...
    value: TValue,
    ctx: RefinementCtx,
  ) => TValue extends string ? z.output<TSchema> : TValue {
    return (value: TValue, ctx: RefinementCtx) => {
      if (value == null) {
        return value;
      }
